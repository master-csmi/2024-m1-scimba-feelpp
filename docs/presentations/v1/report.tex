...
\begin{enumerate}
    ...
    \item Create a docker container with a Feel++ base.
    \item Install dependencies and clone Scimba repository in docker image.
    \item Enable image generation in docker.
    \item Create a devcontainer.
    \item Solve PDEs using Feel++.
    \item Solve PDEs using ScimBa using PINNS.
    \item Create a Poisson class you can call to solve using Feel++.
    \item Add ScimBa as a solver for the class.
    \item Update the Poisson2d class to handle ScimBa with parametrized $f$ and $g$.
    \item Add a diffusion tensor to the Poisson2d class.
    \item Solve different types of PDEs with both solvers with the same parameters.
    \item Compare the results of both solvers.
\end
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/roadmapV0.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/roadmapV1.png}
    \captionsetup{font={scriptsize}}
