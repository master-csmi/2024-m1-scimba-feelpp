...
from tools.GmeshReader import mesh2d
...
rhs_values = ['8*pi*pi*sin(2*pi*x) * sin(2*pi*y)',
              '4* pi * (-cos(pi* (x*x + y*y)) + pi * (x*x + y*y)* sin(pi* (x*x + y*y)))', 
              '-1.0-1*y*x+y*y'] 
...
for geofile in geofile_values:
    for h in h_values:
        for rhs, g in rhs_values, g_values:
            ...
            np.save(filepath, poisson_u, delimiter=',')
